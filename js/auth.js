/* ============================================================
   SUBMANGA - Auth + Wallet (Gmail via Firebase Authentication)
   Login credential = email + password (Firebase Auth).
   Username = public handle (unique, a-z 0-9 _).
   Data isolation: each user lives under `submanga.users/{uid}` + local
   `submanga.user.{uid}.*`. Passwords stay with Firebase only — never
   saved locally.
   NOTE: the Email/Password provider must be ON in the Firebase console
   (Authentication → Sign-in method) and the domain must be authorized.
   ============================================================ */
